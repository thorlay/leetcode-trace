  ...
  return (
    <main className="shell session-page">
      ...
      <header className="session-header">
        <div><p className="eyebrow">解题记录 · {session.problem.slug}</p><h1>{title}</h1><div className="tag-row"><span>LeetCode</span><span>{session.attempts[0]?.language ?? "代码"}</span><span>{session.captureCompleteness === "FULL" ? "完整实时记录" : session.captureCompleteness === "PARTIAL_LIVE" ? "部分实时记录" : session.captureCompleteness === "FINAL_ONLY" ? "仅最终提交" : "仅提交快照"}</span><span>{session.trajectoryStatus === "ANALYZED" ? "轨迹已分析" : session.trajectoryStatus === "AVAILABLE" ? "轨迹可分析" : "轨迹证据有限"}</span>{session.problem.slug === "subarray-sum-equals-k" && <span>前缀和</span>}</div></div>
