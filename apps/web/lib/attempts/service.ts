...
export async function persistAttempt(input: IngestAttemptInput): Promise<IngestedAttempt> {
  for (let attempt = 0; attempt < 3; attempt += 1) {
    try {
      ...
      if (result.sequenceNumber >= 2) await prisma.problemSession.updateMany({ where: { id: result.sessionId, trajectoryStatus: { not: "ANALYZED" } }, data: { trajectoryStatus: "AVAILABLE" } });
      return result;
    } catch (error) {
