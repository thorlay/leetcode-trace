...
export async function buildBackup(options: ExportOptions) {
  ...
  return {
    ...
    sessions: sessions.map((session) => ({
      ...
      status: session.status, analysisStatus: session.analysisStatus, captureCompleteness: session.captureCompleteness, trajectoryStatus: session.trajectoryStatus, initialAssessment: session.initialAssessment, solutionConsulted: session.solutionConsulted,
