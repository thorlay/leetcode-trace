...
export function getSessionOutcome(status: string, attempts: Array<{ action: string; verdict: string | null }>) {
  const unsuccessfulSubmissionCount = attempts.filter((attempt) => attempt.action === "SUBMIT" && attempt.verdict !== null && attempt.verdict !== "UNKNOWN" && attempt.verdict !== "ACCEPTED").length;
  const accepted = status === "SOLVED" || attempts.some((attempt) => attempt.action === "SUBMIT" && attempt.verdict === "ACCEPTED");
